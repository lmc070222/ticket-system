...
#pragma pack(push, 1)
  struct NodePage {
    ...
    KeyType keys[M + 2];
    uint32_t parent;
    char value_or_child_data[(M + 2) * (sizeof(ValueType) > sizeof(uint32_t) ? sizeof(ValueType) : sizeof(uint32_t))];
    uint32_t next;
    
    NodePage() : is_leaf(false), key_num(0), parent(0), next(0) {
      memset(value_or_child_data, 0, sizeof(value_or_child_data));
    }
    
    ValueType& get_value(int idx) {
      return *reinterpret_cast<ValueType*>(value_or_child_data + idx * sizeof(ValueType));
    }
    
    uint32_t& get_child(int idx) {
      return *reinterpret_cast<uint32_t*>(value_or_child_data + idx * sizeof(uint32_t));
    }
    
    void set_value(int idx, const ValueType& val) {
      *reinterpret_cast<ValueType*>(value_or_child_data + idx * sizeof(ValueType)) = val;
    }
    
    void set_child(int idx, uint32_t child) {
      *reinterpret_cast<uint32_t*>(value_or_child_data + idx * sizeof(uint32_t)) = child;
    }
  };
#pragma pack(pop)
