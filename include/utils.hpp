...
#pragma pack(pop)
  ...
  public:
    ...
    FileManager(const std::string &filename) : fname(filename) {
  ...
  sjtu::vector<ValueType> find_by_index(const KeyType &index) {
    ...
    while (true) {
      ...
      for (int i = 0; i < cur_node.key_num; ++i) {
        if (cur_node.keys[i] == index) {
          ans.push_back(cur_node.values[i]);
  ...
  void search(uint32_t nextpageno, KeyType &index, ValueType &value) {
    NodePage nextpage;
...
void quick_sort_helper(sjtu::vector<T> &arr, int left, int right,
                       Compare comp) {
  if (left >= right)
    return;
  int i = left;
  ...
  if (left < j)
    quick_sort_helper(arr, left, j, comp);
  if (i < right)
    quick_sort_helper(arr, i, right, comp);
}
template <typename T, typename Compare>
void sort(sjtu::vector<T> &arr, Compare comp) {
  if (arr.size() <= 1)
    return;
  quick_sort_helper(arr, 0, arr.size() - 1, comp);
}
